...
async function processFiles(files) {
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            // Read file content immediately when adding to the list
            let fileContent;
            
            if (file instanceof File) {
                // Handle File objects from input element
                fileContent = await readFileAsText(file);
            } else if (file.path) {
                // Handle file paths from Neutralino API
                try {
                    const fileData = await Neutralino.filesystem.readBinaryFile(file.path);
            ...
            fileList.push({
                name: file.name,
                content: fileContent,  // Store content instead of path
...
async function createTxtOutputFile(folderPath, fileNumber) {
    ...
    for (const file of fileList) {
        // Add file content to the output using stored content
...
`;
        ...
        console.log(`Added file content: ${file.name}`);
    }
