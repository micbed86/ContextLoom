...
async function handleDrop(e) {
    ...
    if (dt.files.length > 0) {
        // In Neutralino, we need to use the native file dialog to get the actual file paths
        // since drag and drop doesn't provide complete file path information
        try {
            // Use Neutralino's native file dialog to get the actual file paths
            const entries = await Neutralino.os.getDraggedFiles();
            
            if (entries && entries.length > 0) {
                // Process the files with proper paths
                ...
                updateStatus('No valid files found in drop', 'warning');
            }
        } catch (error) {
...
async function processFiles(files) {
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            // For Neutralino, we need to handle files differently than in a browser
