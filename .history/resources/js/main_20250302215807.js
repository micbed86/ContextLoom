...
function updateUILanguage(lang) {
    ...
    // Fix checkbox labels - using a more compatible approach
    const formatOptions = document.querySelectorAll('.format-options label');
    formatOptions.forEach(label => {
        const input = label.querySelector('input');
        if (input && input.id === 'txtFormat') {
            // Keep the checkbox but replace the text
            const checkbox = input.cloneNode(true);
            label.innerHTML = '';
            label.appendChild(checkbox);
            label.appendChild(document.createTextNode(' ' + texts.textFormat));
        } else if (input && input.id === 'htmlFormat') {
            // Keep the checkbox but replace the text
            const checkbox = input.cloneNode(true);
            label.innerHTML = '';
            label.appendChild(checkbox);
            label.appendChild(document.createTextNode(' ' + texts.htmlFormat));
        }
    });
